...
字典序小： 比如 字符串， b ， ba 那么只有组成 bab 才是最小的。

思想：一开始我们可能想到，如果比较一个数组中以单个字符串的大小排好序，并且这个组成的字符串是最小的，但是经过实验，这个贪心策略是错的。

正解：是比较字符串中，两个字符拼接在一起，比较顺序，并且以此排好序，那么这个字符串数组组成的字符串就是最小，经过实验，这个贪心策略是对的。

算法： a.b  <= b.a      并且，  b.c  <= c.b   那么a.c < = c.a. 这是整数类型的传递性。 a.b 就是以a 和 b 组成的字符。
